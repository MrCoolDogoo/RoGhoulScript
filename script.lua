...
local myData = loadstring(game:HttpGet("https://raw.githubusercontent.com/MrCoolDogoo/RoGhoulScript/refs/heads/main/Settings.lua"))()("script.lua", {
    Skills = {
        E = false,
        F = false,
        C = false,
                    ...
                    if not array.found then
                        while findobj(findobj(npc.Parent, npc.Name), "Head") and player.Character.Humanoid.Health > 0 and array.autofarm do
                            ...
                            if myData.Boss[npc.Name] or npc.Parent.Name == "GyakusatsuSpawn" then 
                                ...
                                player.Character.HumanoidRootPart.CFrame = npc.HumanoidRootPart.CFrame + Vector3.new(0,-1*myData.DistanceFromBoss ,0)
                            else
